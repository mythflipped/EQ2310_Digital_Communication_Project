...
BER_theory =  qfunc(sqrt(2*10.^(EbN0_db/10)));

plot(EbN0_db,BER_theory);
set(gca, 'YScale', 'log');

xlabel('SNR(dB)')
ylabel('BER')
legend('Simulation Value','Theoritical Value')
...
%% Problem 3: Signal constellation
for snr_point = 1:length(EbN0_db)
 for blk = 1:nr_blocks
...
%% Problem 4: PSD
%Change the pulse shape then run this code
figure;
pwelch(tx);


%% Problem 5: Eye diagram
figure;
eyediagram(r,4);
...
%% Prblem 7: Time-varying phase shift



%% Problem 8: Differential modulation/demodulation


%% Problem 9: Better training sequences
