...
% Initialization
EbN0_db = 20;                     % Eb/N0 values to simulate (in dB)
...
nr_errors = zeros(1, 25);   % Error counter
for nr_training_bits = 2:2:50             % Size of training sequence (in nr bits)
  ...
  for blk = 1:nr_blocks
    ...
    nr_errors(nr_training_bits/2) = nr_errors(nr_training_bits/2) + sum(temp);
...
BER_db=10*log10(BER);
figure
scatter(2:2:50,BER_db)
title(['Experiment BER with E_b/N_0=', num2str(EbN0_db), 'dB'])
xlabel('# of training bits')
ylabel('BER/dB')
